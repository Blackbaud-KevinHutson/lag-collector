...
function columnTemplate (content, width) {
  ...
  return `<div class="span${width} mt-2 mb-2 ml-2 mr-2">${content}</div>`;
}

function listGroupTypeClass (lagNumber) {
  if (lagNumber === 0) {
    return 'success';
...
function listGroupItem (app, topic, lag) {
  const groupType = listGroupTypeClass(lag);
  let nameTag = `chart-${app}-${topic}`;
  return `<li class="list-group-item list-group-item-${groupType}"><small><a href="charts/${app}.htm#${nameTag}">${topic}</a> (${lag})</small></li>`;
}
...
function cardTemplate (cardTitle, cardText) {
  return `<div class="card"><div class="card-body">${cardTitle}${cardText}</div></div>`;
}
