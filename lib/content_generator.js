...
// Squash lagForApp into a row per topic for each app, summing the lag across partitions
// This can be used for logging/alerting purposes for total lag per topic
function logTotalLagForTopicByApp(lagForApp) {
  let byTopic = _.groupBy(lagForApp, 'TOPIC');
  let lagItems = [];
  let consumerName = '';
  let lagTime = 0;
  _.each(Object.keys(byTopic), (topic) => {
    ...
    log.log('DEBUG', `consumer_name=${consumerName} topic=${topic} totalTopicLag=${totalLag} lagTime=${lagTime}`);
  });
  return lagItems;
}
...
function generatePages(lagRows) {
  ...
  _.each(consumerGroups, (app) => {
    ...
    logTotalLagForTopicByApp(lagForApp);
  });
}

module.exports = { generatePages, logTotalLagForTopicByApp };  
