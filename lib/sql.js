...
function query_apps_and_topics(dbFilePath, cb) {   
    let db = openDatabase(dbFilePath);
    let sql = "SELECT consumer_name, topic FROM consumer group by consumer_name, topic";

    db.all(sql, [], (err, rows) => {
        if (err) {
            console.log("query_apps_and_topics err=" + JSON.stringify(err));
        }
        cb(rows);
    });
    closeDatabase(db);     
...
module.exports = { save_lag, query_lag, query_apps_and_topics };  
