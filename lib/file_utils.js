...
function list_files_in_datastore(callback) {
  ...
  stream.once('error', err => {
      log.log('ERROR',`list_files_in_datastore GLOB err=${err}`);
  });
  stream.once('end', () => {
      callback(entries);
  });
...
function delete_expired_data_store_files() {
  log.log('INFO',`delete_expired_data_store_files dataStoreDir=${JSON.stringify(dataStoreDir)}`);
  list_files_in_datastore((filePaths) => {
    _.each(filePaths, (filePath) => {
      fs.stat(filePath, (err, fileMetaData) => {
        if(err) {
          log.log('ERROR', `Enountered an error checking the expiration of a dataStore file. filePath=${filePath} err=${err}`);
        }
        if(is_data_store_file_expired(fileMetaData.mtime)) {
          fs.unlink(filePath, err => {
            if(err) {
              log.log('ERROR', `Enountered an error deleting an expired dataStore file. filePath=${filePath} err=${err}`);
            }
          });
        }  
      });
